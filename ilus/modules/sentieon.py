...
class Sentieon(object):
    ...
    def bwamem(self, out_prefix, rgID, fastq1, fastq2=""):
        ...
        sort_options = " ".join([str(x) for x in self.config["sentieon"].get("sort_options", [])])
    ...
    def alignment_metrics(self, input_bam, out_prefix):
        ...
        """
        metrics_options = " ".join([str(x) for x in self.config["sentieon"].get("alignment_metrics", [])])
    ...
    def indelrealigner(self, input_bam, output_realig_bam):
        ...
        return (f"time {self.sentieon} driver -r {self.reference_fasta} {indel_realigner_options} "
                f"-i {input_bam} --algo Realigner "
    ...
    def baserecalibrator(self, input_bam, out_bqsr_recal_table):
        ...
        bqsr_apply_options = " ".join([str(x) for x in self.config["sentieon"].get(
            "bqsr_apply_options", [])])

        # Create recalibrate table file for BQSR
        recal_table_cmd = (f"time {self.sentieon} driver "
                           f"-r {self.reference_fasta} {bqsr_recaltable_options} "
                           f"-i {input_bam} --algo QualCal "
                           f"-k {dbsnp} "
                           f"-k {known_Mills_indels} "
        ...
        apply_bqsr_cmd = (f"time {self.sentieon} driver -r {self.reference_fasta} {bqsr_apply_options} "
                          f"-i {input_bam} -q {out_bqsr_recal_table} --algo QualCal "
                          f"-k {dbsnp} "
                          f"-k {known_Mills_indels} "
        ...
        plot_cmd = (f"time {self.sentieon} driver --algo QualCal --plot "
                    f"--before {out_bqsr_recal_table} "
                    f"--after {recal_table_post_fn} {recal_report_table} && "
                    f"{self.sentieon} plot bqsr -o {bqsrreport_plot_fn} {recal_report_table}")
    ...
    def variantrecalibrator(self, input_vcf, output_vcf_fname):
        ...
        snp_vqsr_cmd = (
            f"time {self.sentieon} driver -r {self.reference_fasta} --algo VarCal "
            f"-v {input_vcf} "
        ...
        apply_snp_vqsr_cmd = (f"time {self.sentieon} driver -r {self.reference_fasta} "
                              f"--algo ApplyVarCal -v {input_vcf} "
                              f"--var_type SNP "
                              f"--recal {out_prefix}.SNPs.recal "
                              f"--tranches_file {out_prefix}.SNPs.tranches.csv "
                              f"{apply_snp_vqsr_options} "
        ...
        apply_indel_vqsr_cmd = (f"time {self.sentieon} driver -r {self.reference_fasta} "
                                f"--algo ApplyVarCal -v {out_snp_vqsr_fname} "
                                f"--var_type INDEL "
                                f"--recal {out_prefix}.INDELs.recal "
                                f"--tranches_file {out_prefix}.INDELs.tranches.csv "
                                f"{apply_indel_vqsr_options} "
